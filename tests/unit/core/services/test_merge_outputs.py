...
def test_main_writes_bundle_and_stdout(tmp_path: Path, capsys) -> None:
    ...
    with (
        patch("adrift.core.services.app_common.load_podcasts_config", return_value=[config]),
        patch(
            "adrift.core.services.catalog.merge_config", return_value=result
...
def test_main_updates_output_file_after_each_podcast(tmp_path: Path, capsys) -> None:
    ...
    with (
        patch("adrift.core.services.app_common.load_podcasts_config", return_value=[first, second]),
        patch(
            "adrift.core.services.catalog.merge_config", side_effect=[first_result, second_result]
        ),
...
def test_main_defaults_output_dir_to_downloads(tmp_path: Path, capsys) -> None:
    ...
    with (
        patch("adrift.core.services.app_common.load_podcasts_config", return_value=[config]),
        patch("adrift.core.services.catalog.merge_config", return_value=result),
...
def test_main_emits_timings_to_stderr(tmp_path: Path, capsys) -> None:
    ...
    with (
        patch("adrift.core.services.app_common.load_podcasts_config", return_value=[config]),
        patch("adrift.core.services.catalog.merge_config", side_effect=_merge_config_with_timings),
        patch.object(sys, "argv", argv),
    ):
        merge_mod.main()
