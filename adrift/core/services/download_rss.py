...
def _build_channel(config: PodcastConfig, source_factory: EpisodeSourceFactoryPort) -> RssChannel:
    channel = RssChannel(
        title=config.name, author="", subtitle="", url="", description="", image=""
    )
    for source in config.references:
        try:
            _fill_channel(channel, source_factory.get(source).fetch_channel(source))
