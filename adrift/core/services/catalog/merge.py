...
class MergeConfigOptionOverrides(TypedDict, total=False):
    ...
    episode_source_factory: EpisodeSourceFactoryPort | None
    alignment_provider: AlignmentBackendProviderPort | None
...
def _resolved_collector_port(options: MergeConfigOptions) -> EpisodeCollectorPort:
    ...
    return LegacyEpisodeCollectorAdapter(
        ...
        source_factory=options.episode_source_factory,
    )
...
def _resolved_scored_alignment_port(
    ...
    if options.alignment_provider is not None:
        return options.alignment_provider.get()
    return None
...
def _resolved_alignment_candidate_port(
    ...
    if backend is None or options.alignment_provider is None:
        return None
    return options.alignment_provider.get(backend)
