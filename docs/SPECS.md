...
- Lint gates: `tests/test_lint.py` runs ruff (check + format), ty, Lizard (CCN $\le 8$, length $\le 30$; `adrift/cli/*` excluded), jscpd copy-paste detection, Semgrep architecture rules (`rules/semgrep`), and Vulture dead-code detection.
